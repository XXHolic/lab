window.onload = function () {
  const page = {
    ...
    initBuffersForScreen: function (gl) {
      const vertices = new Float32Array([0, 0.5, -0.3, -0.4, 0.3, -0.4]); // 三角形
    ...
    draw: function () {
      ...
      const transformValue = this.getTransform(this.transformData);
    ...
    pageEvent: function () {
      const rEle = document.querySelector("#rTransform");
      rEle.onchange = (e) => {
        const value = e.target.value;
        console.info("旋转：", value);
        this.transformData = Number(value);
        this.draw();
      };
    },
  };
