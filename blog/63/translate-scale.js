const page = {
  ...
  draw: function() {
    ...
    const calculate = Math.cos(this.scale) * 2;
    // const calculate = 2;
    Util.CANVAS.resetTransform(context)
    let fixParams = { context,...fixAttr };
    Util.CANVAS.translate(context,90,75);
    Util.CANVAS.scale(context,calculate,calculate);
    const fixNewPoints = this.getPosition(fixAttr);
    // console.info('newPoints',fixNewPoints)
    Util.CANVAS.drawLine(fixParams);

    Util.CANVAS.resetTransform(context)

    let moveParams = { context,...moveAttr };
    Util.CANVAS.translate(context,180,75);
    Util.CANVAS.scale(context,calculate,calculate);
